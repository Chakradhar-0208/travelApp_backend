...
router.get("/users", authenticateToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        if (cachedData) {
            ...
            return res.status(200).json({ users: cachedData, source: "cache" });
        }
        const users = await User.find({}, { name: 1, email: 1, role: 1, profileImage: 1, tripCount: 1, travelType: 1, interests: 1, status: 1 }).sort({ role: 1, createdAt: -1 }).lean(); //only returns specific fields for performance imp

        setCache(cacheKey, users);
        // console.log("Cache Set: ", cacheKey);
        res.status(200).json({ users: users, source: "db" });
    } catch (err) {
...
router.get("/reports", authenticateToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        const reports = await Report.find()
            .populate("target", "name email title")   // User, Trip, Review — works with refPath
            .populate("reportedBy", "name email")    // this one exists in schema
            .lean();

        setCache(cacheKey, reports);

        return res.status(200).json({ reports, source: "db" });
